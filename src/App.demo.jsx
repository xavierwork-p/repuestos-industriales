...
import { clearDemoLocalChanges, supabase } from './lib/demoSupabaseClient'
...
const importHeaderAliases = {
  ...
  cliente_referencia: 'client_reference',
  cliente_o_referencia: 'client_reference',
  cliente_solicitud: 'client_reference',
  client_reference: 'client_reference',
  producto_referencia: 'product_reference',
  producto_o_referencia: 'product_reference',
  producto_solicitud: 'product_reference',
...
function findClientForImport(clients, reference) {
  ...
  return clients.find((client) => {
    ...
    return values.some((value) => String(value || '').trim().toLowerCase() === cleanReference)
  }) || null
}

function findProductForImport(products, reference) {
  const cleanReference = String(reference || '').trim().toLowerCase()
  if (!cleanReference) return null
...
function buildRequestImportCandidate(row, clients, products, statuses) {
  const data = row.data
  const warnings = []
  const errors = []

  const clientReference = data.client_reference || data.company_name || data.contact_name || data.phone || data.email || data.reference
  const productReference = data.product_reference || data.name || data.description || data.manufacturer_code || data.internal_code || data.reference

  if (!clientReference) errors.push('La solicitud no tiene cliente o referencia de cliente.')
  if (!productReference) errors.push('La solicitud no tiene producto o referencia de producto.')

  const matchedClient = findClientForImport(clients, clientReference)
  const matchedProduct = findProductForImport(products, productReference)

  if (clientReference && !matchedClient) warnings.push('Cliente no encontrado; se creará un cliente temporal.')
  if (productReference && !matchedProduct) warnings.push('Producto no encontrado; se creará un producto temporal.')

  const quantity = Number(data.quantity || 1)
  if (!data.quantity) warnings.push('Cantidad vacía; se usará 1.')
  if (Number.isNaN(quantity) || quantity <= 0) errors.push('La cantidad debe ser mayor que cero.')

  const requestDate = normalizeImportDate(data.request_date)
  if (!data.request_date) warnings.push('Fecha vacía; se usará la fecha actual.')

  const defaultStatus = statuses.find((status) => status.name === 'Solicitado') || statuses[0]
  const matchedStatus = statuses.find(
    (status) => status.name.toLowerCase() === String(data.status_name || '').trim().toLowerCase()
  )

  if (!data.status_name) warnings.push('Estado vacío; se usará Solicitado.')
  if (data.status_name && !matchedStatus) warnings.push('Estado no reconocido; se usará Solicitado.')
  ...
  return {
    ...
    payload: {
      ...
      status_id: matchedStatus?.id || defaultStatus?.id || '',
      status_name: matchedStatus?.name || defaultStatus?.name || 'Solicitado',
      comments: data.comments || '',
    },
    preview: {
      main: `${clientReference || 'Cliente sin referencia'} → ${productReference || 'Producto sin referencia'}`,
      secondary: `Cantidad: ${quantity || 1} · Estado: ${matchedStatus?.name || defaultStatus?.name || 'Solicitado'} · Fecha: ${requestDate}`,
    },
  }
}
...
function DemoBanner() {
  function handleResetDemo() {
    const confirmed = window.confirm(
      'Esto limpiara los cambios locales de esta demo en este navegador. La base demo no se modifica.'
    )

    if (confirmed) clearDemoLocalChanges()
  }

  return (
    <div style={styles.demoBanner}>
      <div>
        <strong style={styles.demoBannerTitle}>DEMO</strong>
        <span style={styles.demoBannerText}>
          Cambios locales.
        </span>
      </div>

      <button type="button" style={styles.demoBannerButton} onClick={handleResetDemo}>
        Reiniciar demo
      </button>
    </div>
  )
...
function App() {
  ...
  function openClientHistory(client) {
    setHistoryModal({ type: 'client', entity: client })
  }

  function openProductHistory(product) {
    setHistoryModal({ type: 'product', entity: product })
  }
...
function openNewProductForm() {
  ...
  async function downloadImportTemplate() {
    ...
    instructions.addRows([
      ...
      { section: 'Orden', instruction: 'Importa primero clientes, luego productos y finalmente solicitudes.' },
      { section: 'Solicitudes', instruction: 'La hoja Solicitudes puede usar el nombre de cliente/producto existente. Si no existe, el sistema creará registros temporales con advertencia.' },
    ])
      ...
      [
        {
          ...
          request_date: new Date().toISOString().slice(0, 10),
          status: 'Solicitado',
          comments: 'Ejemplo listo para importar después de clientes y productos.',
        },
      ]
    )
  ...
  async function handleImportFileChange(event) {
    ...
    try {
      ...
      const candidates = rows.map((row) => {
        ...
        if (importType === 'products') return buildProductImportCandidate(row)
        return buildRequestImportCandidate(row, clients, products, statuses)
      })
  ...
  async function confirmImport() {
    ...
    try {
      if (importType === 'clients') {
        ...
        for (const row of importPreviewRows) {
          let clientId = row.payload.client_id
          let productId = row.payload.product_id

          if (!clientId) {
            const { data: createdClient, error: clientError } = await supabase
              .from('clients')
              .insert({
                company_name: `Cliente pendiente ${row.payload.client_reference}`,
                notes: `Creado automáticamente desde importación de solicitudes. Referencia: ${row.payload.client_reference}`,
              })
              .select()
              .single()
          ...
          if (!productId) {
            const { data: createdProduct, error: productError } = await supabase
              .from('products')
              .insert({
                name: `Producto pendiente ${row.payload.product_reference}`,
                observations: `Creado automáticamente desde importación de solicitudes. Referencia: ${row.payload.product_reference}`,
              })
              .select()
              .single()
          ...
          const { data: createdRequest, error: requestError } = await supabase
            ...
            .insert({
              ...
              request_date: row.payload.request_date,
              status_id: row.payload.status_id,
          ...
          await supabase.from('request_status_history').insert({
            ...
            old_status_id: null,
            new_status_id: row.payload.status_id,
            comment: 'Solicitud importada desde Excel',
          })
        ...
        await loadRequests()
        await loadDashboard()
        showSuccess(`${importedCount} solicitudes importadas correctamente.`)
      }
...
/>
  ...
  if (loading) {
    return (
      <>
        <DemoBanner />
        <p style={{ ...styles.loading, ...styles.loadingDemo }}>Cargando sistema...</p>
      </>
    )
  }
  ...
  if (!session) {
    return (
      <>
        <DemoBanner />
        <div style={{ ...styles.loginPage, ...styles.loginPageDemo }}>
  ...
  return (
    <>
      <DemoBanner />
      <div style={{ ...styles.app, ...styles.appDemo, ...(isMobile ? styles.appMobile : {}) }}>
...
function ReportsPage({
  ...
  const filteredRequests = useMemo(() => {
    return requests.filter((request) => {
      const matchesFrom = !reportFilters.from || request.request_date >= reportFilters.from
      const matchesTo = !reportFilters.to || request.request_date <= reportFilters.to
      const matchesClient = !reportFilters.clientId || request.client_id === reportFilters.clientId
      const matchesProduct = !reportFilters.productId || request.product_id === reportFilters.productId
      const matchesStatus = !reportFilters.statusName || request.status_name === reportFilters.statusName
      const matchesCategory = !reportFilters.categoryName || request.category_name === reportFilters.categoryName
      const matchesBrand = !reportFilters.brand || request.brand === reportFilters.brand
  ...
  }, [requests, reportFilters])

  const summary = useMemo(
    () => buildReportSummary({ clients, products, requests: filteredRequests }),
    [clients, products, filteredRequests]
  )
  ...
  async function exportFilteredExcel() {
    ...
    const buffer = await workbook.xlsx.writeBuffer()
    downloadBlob(new Blob([buffer]), `reporte-filtrado-${getCurrentDateFileName()}.xlsx`)
  }

  function exportFilteredPdf() {
    const doc = new jsPDF({ orientation: 'landscape' })
    doc.setFontSize(16)
    doc.text('Reporte filtrado de solicitudes', 14, 16)
    doc.setFontSize(10)
    doc.text(`Generado: ${new Date().toLocaleString('es-DO')}`, 14, 23)
    doc.text(`Solicitudes filtradas: ${filteredRequests.length}`, 14, 30)

    autoTable(doc, {
      startY: 36,
      head: [['Fecha', 'Cliente', 'Producto', 'Marca', 'Categoría', 'Cantidad', 'Estado']],
    ...
    doc.save(`reporte-filtrado-${getCurrentDateFileName()}.pdf`)
  }
    ...
    <div>
      ...
      <section style={styles.panel}>
        <div style={{ ...styles.sectionHeader, ...(isMobile ? styles.sectionHeaderMobile : {}) }}>
          <div>
            ...
            <p style={styles.emptyText}>
              Filtra la demanda por fecha, cliente, producto, estado, categoría o marca.
            </p>
          </div>
        ...
        <div style={{ ...styles.filtersGrid, ...(isMobile ? styles.filtersGridMobile : {}) }}>
          <FormInput label="Desde" name="from" type="date" value={reportFilters.from} onChange={handleReportFilterChange} />
          <FormInput label="Hasta" name="to" type="date" value={reportFilters.to} onChange={handleReportFilterChange} />

          <div>
            <label style={styles.label}>Cliente</label>
            <select style={styles.input} name="clientId" value={reportFilters.clientId} onChange={handleReportFilterChange}>
          ...
          <div>
            <label style={styles.label}>Producto</label>
            <select style={styles.input} name="productId" value={reportFilters.productId} onChange={handleReportFilterChange}>
              <option value="">Todos</option>
              {products.map((product) => (
          ...
          <div>
            <label style={styles.label}>Estado</label>
            <select style={styles.input} name="statusName" value={reportFilters.statusName} onChange={handleReportFilterChange}>
              <option value="">Todos</option>
              {statuses.map((status) => (
          ...
          <div>
            ...
            <select style={styles.input} name="categoryName" value={reportFilters.categoryName} onChange={handleReportFilterChange}>
          ...
          <div>
            ...
            <select style={styles.input} name="brand" value={reportFilters.brand} onChange={handleReportFilterChange}>
              <option value="">Todas</option>
        ...
        <div style={{ ...styles.reportExportGrid, ...(isMobile ? styles.reportExportGridMobile : {}) }}>
          <button style={styles.tableButton} onClick={clearReportFilters}>Limpiar filtros</button>
          <button style={styles.primaryButtonSmall} onClick={exportFilteredExcel}>Excel filtrado</button>
          <button style={styles.secondaryButton} onClick={exportFilteredPdf}>PDF filtrado</button>
        ...
        <div style={{ ...styles.cardsGrid, ...(isMobile ? styles.cardsGridMobile : {}) }}>
          ...
          <StatCard title="Solicitudes filtradas" value={summary.totalRequests} />
        </div>
      </section>
      ...
      <section style={{ ...styles.dashboardGrid, ...(isMobile ? styles.dashboardGridMobile : {}) }}>
        <ReportList
          ...
          emptyText="No hay productos solicitados para estos filtros."
        ...
        <ReportList
          ...
          emptyText="No hay clientes activos para estos filtros."
      ...
      <section style={styles.panel}>
        ...
        {summary.monthlySummary.length === 0 ? (
          <p style={styles.emptyText}>No hay solicitudes mensuales para estos filtros.</p>
        ) : (
          <div style={styles.tableWrapper}>
            <table style={styles.table}>
              <thead>
                  ...
                  {importPreviewRows.map((row) => (
                    <tr key={`${row.type}-${row.rowNumber}`}>
                      ...
                      <td style={styles.td}>
                        ...
                        <small style={styles.smallText}>{row.type === 'client' ? 'Cliente' : row.type === 'product' ? 'Producto' : 'Solicitud'}</small>
                      </td>
...
function HistoryModal({ historyModal, requests, onClose }) {
  const { type, entity } = historyModal
  const entityRequests = requests.filter((request) => {
    if (type === 'client') return request.client_id === entity.id
    return request.product_id === entity.id
  })

  const totalQuantity = entityRequests.reduce((total, request) => total + Number(request.quantity || 0), 0)
  const lastRequest = entityRequests
    .slice()
    .sort((a, b) => String(b.request_date || '').localeCompare(String(a.request_date || '')))[0]

  const groupedMap = new Map()

  entityRequests.forEach((request) => {
    const key = type === 'client'
      ? request.product_id || request.product_name || 'sin-producto'
      : request.client_id || request.company_name || 'sin-cliente'

    const current = groupedMap.get(key) || {
      name: type === 'client' ? request.product_name || 'Producto sin nombre' : request.company_name || 'Cliente sin nombre',
      totalRequests: 0,
      totalQuantity: 0,
    }

    current.totalRequests += 1
    current.totalQuantity += Number(request.quantity || 0)
    groupedMap.set(key, current)
  })

  const groupedRows = Array.from(groupedMap.values()).sort((a, b) => b.totalRequests - a.totalRequests)
  const title = type === 'client' ? entity.company_name : entity.name
  const subtitle = type === 'client' ? 'Historial por cliente' : 'Historial por producto'

  return (
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.historyModalContent} onClick={(event) => event.stopPropagation()}>
        <div style={styles.modalHeader}>
          <div>
            <h3 style={styles.modalTitle}>{subtitle}</h3>
        ...
        <div style={styles.historyBody}>
          <div style={styles.historyStatsGrid}>
            <StatCard title="Solicitudes" value={entityRequests.length} />
            <StatCard title="Cantidad total" value={totalQuantity} />
            <StatCard title="Última solicitud" value={lastRequest ? formatDate(lastRequest.request_date) : '—'} />
          </div>

          <div style={styles.panelSoft}>
            <h4 style={styles.formTitle}>{type === 'client' ? 'Productos solicitados' : 'Clientes relacionados'}</h4>
            {groupedRows.length === 0 ? (
              <p style={styles.emptyText}>No hay historial registrado.</p>
            ) : (
              <div style={styles.list}>
                {groupedRows.map((row) => (
                  <div key={row.name} style={styles.listItem}>
                    <strong>{row.name}</strong>
                    <small>Solicitudes: {row.totalRequests} · Cantidad: {row.totalQuantity}</small>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={styles.panelSoft}>
            <h4 style={styles.formTitle}>Detalle de solicitudes</h4>
            {entityRequests.length === 0 ? (
              <p style={styles.emptyText}>No hay solicitudes para mostrar.</p>
            ) : (
              <div style={styles.tableWrapper}>
                <table style={styles.table}>
                  <thead>
                    <tr>
                      <th style={styles.th}>Fecha</th>
                      <th style={styles.th}>{type === 'client' ? 'Producto' : 'Cliente'}</th>
                      <th style={styles.th}>Cantidad</th>
                      <th style={styles.th}>Estado</th>
                  ...
                  <tbody>
                    {entityRequests.map((request) => (
                      <tr key={request.id}>
                        <td style={styles.td}>{formatDate(request.request_date)}</td>
                        <td style={styles.td}>{type === 'client' ? request.product_name || '—' : request.company_name || '—'}</td>
                        <td style={styles.td}>{request.quantity}</td>
                        <td style={styles.td}>{request.status_name || '—'}</td>
                        <td style={styles.td}>{request.comments || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
